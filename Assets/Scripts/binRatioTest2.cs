...
public class binRatioTest2 : MonoBehaviour
{
    //Public variables:
    ...
    public float pIntensity = 1.0f; //intensity of each point's light contribution
    ...
    void ReadCSV()
    {
        ...
        if (File.Exists(path))
        {
            ...
            {
                Debug.Log(line + ", curPointCount = " + currentPointCount);
                if(firstLine){
                    firstLine = false;
                }
                else{
    ...
    private void SendPointsToShader()
    {
        ...
        for (int k = 0; k < boxesAmount; k++)
        {
            if(gridSumsTotal[k] > 0)
            {
                ...
                // //we normalise the total sum
                // Debug.Log("For k = " + k + ", we normalise " + gridSumsTotal[k] + " by dividing by total amount of points");
                // gridSumsTotal[k] = gridSumsTotal[k]/currentPointCount;
                // Debug.Log("Now we have total " + gridSumsTotal[k]);
                // hitData[k] = gridSumsTotal[k]; //first the intensity, with the decimal number
                // //now we add the ratio as whole number
                // Debug.Log("We assign with ratio " + gridSumsRatio[k] + ", by calculation with floor " + Mathf.Floor(gridSumsRatio[k]*100));
                // hitData[k] += Mathf.Floor(gridSumsRatio[k]*100);
                // Debug.Log("This is equal to " + (Mathf.Floor(gridSumsRatio[k]*100)+gridSumsTotal[k]) + " which has ratio " + Mathf.Floor(hitData[k]) + ", and intensity " + (hitData[k] - Mathf.Floor(hitData[k])));
            }
            else
            {
...
// //DEBUG: We add our own data points:
        // hitData[50] = 0.1f;
        // hitData[51] = 0.2f;
        // hitData[52] = 0.3f;
        // hitData[53] = 0.4f;
